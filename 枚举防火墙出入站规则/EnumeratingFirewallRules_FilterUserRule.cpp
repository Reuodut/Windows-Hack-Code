...
void DumpFWRulesInCollection(INetFwRule* FwRule)
{
	...
	if (SUCCEEDED(FwRule->get_Grouping(&bstrVal)))
	{
		...
		{
			char* Char_bstrVal = _com_util::ConvertBSTRToString(bstrVal);
			Grouping = Char_bstrVal;
		}

		string StrFilter = ".dll,-";
		string::size_type Idx = Grouping.find(StrFilter);
		if (Idx != string::npos)
			return;

		StrFilter = ".exe,-";
		Idx = Grouping.find(StrFilter);
		if (Idx != string::npos)
			return;
